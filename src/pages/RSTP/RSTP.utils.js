		...
		content: (
			<>
				...
				<p>
					...
					portowi. Można bezpiecznie założyć, że port "listening" jest
					wyznaczony lub root i jest w drodze do stanu "forwarding". Niestety,
					po przejściu do stanu "forwarding" nie ma możliwości wywnioskowania ze
					stanu portu, czy port jest portem głównym, czy wyznaczonym. Przyczynia
					się to do wykazania niepowodzenia tej terminologii państwowej. RSTP
					oddziela rolę i stan portu, aby rozwiązać ten problem. Więcej
					informacji na temat tych portów omówiono w częśći poświęconej{' '}
					<a href="/stp">STP</a>. [1]
				</p>
			</>
		),
		...
		content: (
			<>
				<p>
					...
					BPDU na moście, jest portem głównym (Root Port, RP). Znajduje się
					najbliżej mostu głównego (Root Bridge) pod względem kosztu ścieżki.
					STP wybiera pojedynczy most główny w całej zmostkowanej sieci (dla
					każdej sieci VLAN). Most główny wysyła jednostki BPDU, które są
					bardziej przydatne niż te, które wysyła każdy inny most. Most główny
					jest jedynym mostem w sieci, który nie ma portu głównego. Wszystkie
					inne mosty odbierają jednostki BPDU na co najmniej jednym porcie. [1]
				</p>
				<img src={role1} alt="role1"></img>
				<p>
					<b>Role portów wyznaczonych</b> - Port jest wyznaczony (designated),
					jeśli ma możlwość by wysłać najlepszą jednostkę BPDU w segmencie, do
					którego jest podłączony. Mosty 802.1D łączą ze sobą różne segmenty,
					takie jak segmenty sieci Ethernet, w celu utworzenia domeny z
					mostkiem. W danym segmencie może istnieć tylko jedna ścieżka
					prowadząca do mostu głównego. Jeśli są dwa, w sieci istnieje pętla
					pomostowa. Wszystkie mosty podłączone do danego segmentu nasłuchują
					jednostek BPDU każdego z nich i uzgadniają most, który wysyła
					najlepszą jednostkę BPDU, jako most wyznaczony dla tego segmentu. Port
					na tym moście, który odpowiada, jest portem wyznaczonym dla tego
					segmentu. [1]
				</p>
				<img src={role2} alt="role2"></img>
				<p>
					<b>Role portów alternatywnych i zapasowych</b> - Te dwie role
					odpowiadają stanowi blokowania (discarding) 802.1D. Zablokowany port
					jest definiowany jako port inny niż wyznaczony czy główny. Odbiera on
				...
				<p>
					...
					wykluczony z grupy łącza w górę. [1]
				</p>
			</>
		),
	},
		...
		content: (
			<>
				<p>
					...
					802.1D. RSTP wykorzystuje teraz wszystkie sześć bitów bajtu flagi. [1]
				</p>
